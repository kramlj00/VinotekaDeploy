...
export const signin = (email, password) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      ...
      payload: "Pogrešan email ili lozinka",
    });
  }
};
...
export const updateUserProfile = (user) => async (dispatch) => {
  ...
  try {
    const { data } = await axiosInstance.put(`/users/regular_profile`, user, {
      headers: {
...
export const detailsBusinessUser = () => async (dispatch) => {
  ...
  try {
    const { data } = await axiosInstance.get("/users/business_info", {
...
export const updateBusinessUserProfile = (user) => async (dispatch) => {
  ...
  const userInfo = await JSON.parse(localStorage.getItem("userInfo"));
  try {
    const { data } = await axiosInstance.put(`/users/business_profile`, user, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`,
      },
    });
